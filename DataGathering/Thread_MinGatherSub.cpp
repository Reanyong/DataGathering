...
int CThread_MinGatherSub::Run()
{
    ...
    do
    {
        ...
        // 이벤트 대기 시간 제한 - INFINITE 대신 타임아웃 사용
        ...
        if (waitResult == WAIT_TIMEOUT) {
            ...
            // 무한 대기 대신 최대 30초까지만 대기
            DWORD maxWaitTime = 30000; // 30초
            ...
            // 30초 경과 후에도 이벤트가 신호 상태가 되지 않으면 강제로 진행
            if ((GetTickCount() - startTime) >= maxWaitTime) {
                CString timeoutMsg = _T("삭제 대기 시간 초과 - 강제로 수집 작업 재개");
                ShowGridDataOutPut(_T("대기 시간 초과"), timeoutMsg);
                SetWriteLogFile(timeoutMsg + " - " + m_strThreadName);

                // 이벤트 상태를 강제로 신호 상태로 설정
