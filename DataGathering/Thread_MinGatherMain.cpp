...
int CThread_MinGatherMain::ExitInstance()
{
	...
	//_WriteLogFile(g_stProjectInfo.szDTGatheringLogPath,"�޸𸮴���Log","MainTread ExitInstance");
	if (m_bButtonStop == TRUE)
	{
		_addSystemMsg(LOG_MESSAGE_2, USER_COLOR_BLUE, m_strLogTitle, USER_COLOR_BLUE, "Message : [����� ����]");
	}
	StopSubThread();		// SubThread ����
	StopDeleteThread();		// DeleteThread ����
#ifdef _DEBUG
	TRACE("Stop - ���ν����� sub ������  ���� \n");
#endif
	...
	if (m_bButtonStop == TRUE)
	{
		_addSystemMsg(LOG_MESSAGE_2, USER_COLOR_BLUE, m_strLogTitle, USER_COLOR_BLUE, "Message : [ADO ���� ���� �� �Ҹ�]");
	}
#ifdef _DEBUG
	TRACE("ADO ���� ���� �� �Ҹ� \n");
#endif
...
int CThread_MinGatherMain::Run()
{
	...
	((CFormView_TAGGather*)(m_pCtrl))->ShowText_State("������....");
	...
	do
	{
		if (DB_Connect->GetDB_ConnectionStatus() != 1)
		{
			...
			{
				((CFormView_TAGGather*)(m_pCtrl))->ShowText_State("���� ���� : 10���� �����ӽõ�..");
		...
		{
			((CFormView_TAGGather*)(m_pCtrl))->ShowText_State("SELECT ���� ����");
			((CFormView_TAGGather*)(m_pCtrl))->ShowText_Count("-");
			((CFormView_TAGGather*)(m_pCtrl))->ShowText_Processor("-");
			Sleep(1000);
			continue;
		...
		try
		{
			...
			if (currentTime.GetDay() == targetDeleteDay && lastCheckedDay != targetDeleteDay) {
				...
				if (m_pThread_Delete != NULL) {
					...
					SysLogOutPut(m_strLogTitle, _T("�ſ� ���� ������ ���� ��û��"), LOG_COLOR_BLUE);
				}
				else {
					// Delete ������ ���� �� ����
					StartDeleteThread();
			...
			if (nTagToalCount != m_nTagListCount)
			{
				...
				{
					((CFormView_TAGGather*)(m_pCtrl))->ShowText_State("TAG ���� �����");
					((CFormView_TAGGather*)(m_pCtrl))->ShowText_Count("-");
					((CFormView_TAGGather*)(m_pCtrl))->ShowText_Processor("-");
					StopSubThread();
				...
				if (nErrorCheck == ERROR_DB_COM_ERROR)
					...
					g_nCheckTagThreadRun = 1; // 20210301 ksw Ķ���극�̼� ��ư Ȱ��ȭ �÷���
				...
				for (int nI = 0; nI < nTheradCount; nI++)
				{
					...
					//if(stSubTherdInfo[nI].nThreadDataCount != nTagCount) //�̰� �ʿ���°� ����..
					{
						...
						((CFormView_TAGGather*)(m_pCtrl))->ShowListData(strProcessorName, "������", strCount, "����Ʈ ���� ��..");
...
void CThread_MinGatherMain::Release_List_ST_DB_Tag(int nMode)
{
	...
	if (nMode == ST_LIST_RELEASE) // ksw 20210316 ���߹迭 list �����Ҵ� ����
	{
...
int CThread_MinGatherMain::GetTagList(const char* szDeviceItem, int nTheradCount, int nDBType, const char* szDBName)
{
	...
	//2020-02-12 jsh : postgre �߰�
	else if (nDBType == DB_POSTGRE)
	{
		if (strlen(szDBName) > 1) //20210702 ksw ����
	...
	//20200225 ����ȣ ���� group_name ����
	//strQuery.Format("SELECT TAG_ID,TAG_NAME,GROUP_NAME,TAG_TYPE FROM %s %s AND GATHER_USE_YN = 0 and TAG_ID = '������û�� ����Ʈ�׸���.����Ʈ�׸���DB����.AI.PCS_AC_ACTIVE_POWER'",strDBName,szDeviceItem);
	...
	if (nDBType == DB_POSTGRE)  //20210302 ksw POSTGRE �б�
	{//DB_POSTGRE
		strQuery.Format("SELECT TAG_NAME,TAG_TYPE FROM %s %s GATHER_USE_YN = 0 or GATHER_USE_YN IS NULL ", strDBName, szDeviceItem);
		//20200220 ����ȣ ���� PostgreSQL odbc �κ� select �߰�
		try
		{
			...
			if (retcode < 0)	//20210309 ksw Select �� ����ó��
				throw retcode;
		...
		catch (SQLRETURN rt) //20210305 ksw  njh ���� ����ó��
		{
	...
	else if (nDBType == DB_MSSQL) //20210302 ksw MSSQL �б�
	{
		if (m_nProduct == 0) //20210310 ksw SELECT�� �б�
			strQuery.Format("SELECT TAG_NAME,TAG_TYPE FROM %s %s GATHER_USE_YN = 0 or GATHER_USE_YN IS NULL ", strDBName, szDeviceItem);
		else
			...
			if (pRs != NULL)
			{
				if (!pRs->GetEndOfFile())
				{
					...
					for (int nSTListCnt = 0; nSTListCnt < nTheradCount; nSTListCnt++)
					{
						...
						while (1)
						{
							if (m_nProduct != 0) //20210310 ksw �б� �߰�
							{
...
int CThread_MinGatherMain::GetTagTotalCountCheck(const char* szDeviceItem, int nDBType, const char* szDBName)
{
	...
	//20200219 ����ȣ PostgreSQL odbc select �κ� �߰�
	//	strQuery.Format("SELECT COUNT(TAG_NAME) as cnt FROM %s %s AND GATHER_USE_YN = 0 AND sTAG_ID = '������û�� ����Ʈ�׸���.����Ʈ�׸���DB����.AI.PCS_AC_ACTIVE_POWER'",strDBName,szDeviceItem);
